...
function to_string(date, with_time = false) {
    ...
    const vals = get_date_values(date).map((val, i) => {
        if (i === 1) {
            // add 1 for month
            val = val + 1;
        }
...
function format(date, format_string = 'YYYY-MM-DD HH:mm:ss') {
    const values = get_date_values(date).map(d => padStart(d, 2, 0));
    const format_map = {
        YYYY: values[0],
...
function today() {
    const vals = get_date_values(new Date()).slice(0, 3);
    return new Date(...vals);
}
...
export { add, format, diff, clone, get_days_in_month, now, today, start_of };
